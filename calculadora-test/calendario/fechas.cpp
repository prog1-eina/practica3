...
int diaDeLaSemana(int dia, int mes, int agno) {
    // Se cuentan los días que han pasado desde el 1-1-1900 hasta el dia-mes-agno.
    int cuentaDias = 0;

    // Días por años completos anteriores a «agno»
    for (int a = AGNO_INICIAL; a < agno; a++) {
        cuentaDias += diasDelAgno(a);
    }

    // Días del año actual
    cuentaDias += diaEnElAgno(dia, mes, agno) - 1;

    /* Se calcula el resultado: como el 1-1-1900 fue lunes, cuentaDias / 7
     * representa el número de semanas completas transcurridas desde el 1-1-1900;
     * cuentaDias % 7, por tanto el número de días transcurridos de la última
     * semana no completa y, debido a la codificación elegida, el día de la semana
     * correspondiente a la fecha dia-mes-agno.
     */
    return cuentaDias % 7;
}
